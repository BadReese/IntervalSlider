...
public class IntervalSlider: UIView {
  ...
  private var defaultThumbImage: UIImage {
    let thumbView = UIView(frame: CGRect(x: 0, y: 0 , width: 20, height: 20))
  ...
  private func build(options: [IntervalSliderOption]?) {
    ...
    self.slider.addTarget(self, action: #selector(self.endDrag), for: .touchUpInside)
    self.slider.addTarget(self, action: #selector(self.endDrag), for: .touchUpOutside)
    ...
    for source in self.sources {
      ...
      if self.isAddMark {
        let mark = UIView(frame: CGRect(x: 0, y: 0, width: 1, height: 8))
        mark.backgroundColor = UIColor.lightGray
        self.marks.append(mark)
        self.insertSubview(mark, belowSubview:self.slider)
      }
    }
    self.seek(value: self.adjustSliderValue(self.defaultValue))
  }
  
  private func layout(){
    self.slider.frame = CGRect(x: 0, y: self.frame.height / 2, width: self.frame.width, height: self.frame.height / 2)
    var index: Int = 0
    for source in self.sources {
  ...
  */
  ...
  private func adjustSliderValue(_ baseValue: Float) -> Float {
